...
int main() {
	...
	unsigned int aliasTexture;
	glGenTextures(1, &aliasTexture);
	glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, aliasTexture);
	glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, 4, GL_RGB, WINDOW_WIDTH, WINDOW_HEIGHT, GL_TRUE);

	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D_MULTISAMPLE, aliasTexture, 0);
	...
	unsigned int nonMultiSamTex;
	glGenTextures(1, &nonMultiSamTex);
	glBindTexture(GL_TEXTURE_2D, nonMultiSamTex);
	...
	glBindFramebuffer(GL_FRAMEBUFFER, intermediateFBO);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, nonMultiSamTex, 0);
	unsigned int rboDepthMul;
	glGenRenderbuffers(1, &rboDepthMul);
	glBindRenderbuffer(GL_RENDERBUFFER, rboDepthMul);
	...
	glBindFramebuffer(GL_FRAMEBUFFER, intermediateFBO);
	//glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, texture2, 0);
	glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, rboDepthMul);
	...
	Shader normalShader("AliasingVertexShader.glsl", "AliasingFragmentShader.glsl", "");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		/*texture0.bind();
		quadShader.setUniform1i("texture0", texture0.getTextureUnit());*/
